...
class AdventOfCode2021
{
  ...
  static int PlayBingo_Part2(List<Dictionary<int, int>[,]> boards, List<int> numbers)
  {
        ...
        for (int j = 0; j < 5; ++j)
        {
          for (int l = 0; l < 5; ++l)
          {
            if (boards[i][j, l].ContainsKey(numbers[s]))
            {
              ...
              if (winners.Count == boards.Count && (markedRows[row] == 5 || markedColumns[(row) / 5, l] == 5))
              {
                bingo = true;
                break;
              }
  ...
  static void Main(string[] args)
  {
    ...
    Console.WriteLine("Result part2 = {0}", PlayBingo_Part2(boards, numbers));
  }
